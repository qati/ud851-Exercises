...
public class SettingsFragment extends PreferenceFragmentCompat implements
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        Toast error = Toast.makeText(getContext(), "Enter a valid number: >0 and <=3!", Toast.LENGTH_SHORT);
        String key = getString(R.string.pref_size_key);
        if (preference.getKey().equals(key)){
            String entered = ((String)newValue).trim();
            if (entered.equals("")) entered = "1";
            try{
                float size = Float.parseFloat(entered);
                if (size<=0 || size>3){
                    error.show();
                    return false;
                }
