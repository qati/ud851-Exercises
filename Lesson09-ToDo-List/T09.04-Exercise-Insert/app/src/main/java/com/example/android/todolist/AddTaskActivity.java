...
public class AddTaskActivity extends AppCompatActivity {
    ...
    public void onClickAddTask(View view) {
        ...
        String txt = ((EditText)findViewById(R.id.editTextTaskDescription)).getText().toString();
        if (txt.length()<1){
            return;
        }
        ...
        cv.put(TaskContract.TaskEntry.COLUMN_DESCRIPTION, txt);
        cv.put(TaskContract.TaskEntry.COLUMN_PRIORITY, mPriority);

        Uri res = getContentResolver().insert(TaskContract.TaskEntry.CONTENT_URI, cv);
